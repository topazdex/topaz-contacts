...
contract VeArtProxy is IVeArtProxy {
    ...
    string[5][10] palettes = [
        ["#FFD54F", "#FFCA28", "#FFB300", "#E5A100", "#C68A00"], // golden topaz
        ["#2A2520", "#1F1B17", "#17140F", "#0F0D0A", "#080705"], // obsidian
        ["#FFB74D", "#FF9800", "#F57C00", "#E65100", "#BF360C"], // amber glow
        ["#FFF8E7", "#F8F0D8", "#F0E6C8", "#E8DCB8", "#DFD2A8"], // champagne
        ["#EF5350", "#D32F2F", "#B71C1C", "#8B1A1A", "#5D1212"], // ruby warmth
        ["#A89B8C", "#8D8070", "#736656", "#5A4D3E", "#423628"], // smoky quartz
        ["#F0F465", "#E4E54A", "#CDCC00", "#A8A800", "#7C7B02"], // citrine
        ["#E6A732", "#4A3728", "#D4941E", "#3B2B1A", "#C2820A"], // tiger's eye
        ["#E8A98E", "#D4876A", "#C06848", "#A0522D", "#7A3B1E"], // copper
        ["#C5E17A", "#A8C84A", "#8BAF2E", "#6E9616", "#527D00"] // peridot
    ];
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        '"description": "Topaz is a next-generation AMM that combines the best of Curve, Convex and Uniswap, designed to serve as a central liquidity hub on BNB Chain. Topaz NFTs vote on token emissions and receive bribes and fees generated by the protocol.",',
    ...
    /// @dev Calculates the number of digits before the "decimal point" in an NFT's veTOPAZ balance.
    ///      Input expressed in 1e18 format.
    function numBalanceDigits(uint256 _balanceOf) internal pure returns (int256 digitCount) {
        uint256 convertedveTOPAZvalue = _balanceOf / 1e18;
        while (convertedveTOPAZvalue != 0) {
            convertedveTOPAZvalue /= 10;
            digitCount++;
        }
    }
    ...
    /// @dev Determines the number of lines in the SVG. NFTs with less than 10 veTOPAZ balance have zero lines.
